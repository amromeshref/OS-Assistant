...
from evaluation.config import PARALLEL_EXECUTION_ENABLED, RAG_ENABLED, DEBUG_MODE, DATASET_VERSION
from evaluation.runners.evaluation_graph import EvaluationGraph
from evaluation.runners.pydantic_schemas import (
    DataPointEvaluation,
...
DATASET_DIR = PARENT_DIR / f"src/evaluation/datasets/{DATASET_VERSION}"
...
class EvaluationRunner:
    ...
    def save_evaluation_output(self, data_point_evaluation: DataPointEvaluation, data_point_id: int):
        output_path = EVALUATION_OUTPUT_DIR / f"data-point-{data_point_id}-evaluation.json"
        
        with open(output_path, "w") as f:
            json.dump(data_point_evaluation.model_dump(), f, indent=4)    


    def run_evaluation(self, state: OSAssistantState, data_point_id: int):
        ...
        data_point_evaluation.user_validation_evaluation = evaluation_graph.user_validation_evaluation_result
        ...
        if len(state.command_executions) == 0:
            ...
            for _ in state.command_executions:
                data_point_evaluation.command_execution_analysis_evaluation.append(CommandExecutionEvaluation())
        ...
        if len(state.steps_resolver) == 0:
            ...
            for _ in state.steps_resolver:
                data_point_evaluation.step_resolver_evaluation.append(StepResolverEvaluation())
        ...
        if len(state.command_error_handlers) == 0:
            ...
            for _ in state.command_error_handlers:
                data_point_evaluation.command_error_handling_evaluation.append(CommandErrorHandlingEvaluation())
...
def main():
    ...
    parser.add_argument(
        ...
        type=int,
        required=True,
        help="The ID for the data point"
    )
    ...
    if not parallel:
        ...
        state = runner.run_os_assistant(data_point_details["user_query"], args.data_point_id)
