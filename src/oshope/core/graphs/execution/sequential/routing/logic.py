...
def router(state: OSHopeState):
    """
    Main router function to determine the next node based on the current state of the OS-HOPE.
    """
    ...
    if state.steps_resolver_active:
        ...
        logger.info(
            f"Routing to next resolved step with the index ({current_resolving_step_index})."
        )
        next_resolved_step = state.steps_resolver[-1].resolved_steps[
    ...
    if next_step.dependencies_required and next_step.step_type != "information":
        logger.info(
            f"Next step ({state.current_step_index}) has dependencies, routing to step resolver."
        )
        # save_debug_state(state, "Routing to step resolver due to dependencies.")
        return STEP_RESOLVER_NODE

    logger.info(
        f"Routing to next step ({state.current_step_index}) without dependencies."
    )
...
def route_after_code_execution(state: OSHopeState) -> str:
    """
    Route after code execution based on the execution result and error handling status.
    """
    if state.command_error_handler_active:
        logger.info(
            "Currently in command error handling mode, routing to code error handling node."
        )
