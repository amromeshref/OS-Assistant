from oshope.core.states.oshope_state import OSHopeState, CommandExecution
...
def code_execution_node(state: OSHopeState) -> OSHopeState:
    """
    ...
    if state.command_error_handler_active:
        if not state.command_error_handlers[-1].can_recover:
            ...
            logger.info(
                "Currently in command error handling mode, executing the recovery command."
            )
        ...
        execution_mode = state.planning.plan_steps[
            step_index
        ].step_details.execution_mode
    ...
    command_execution = CommandExecution(
        command=command, success=success, output=output, error=error, summary=summary
    )
    ...
    if not command_execution.success:
        if state.num_error_executions < COMMAND_ERROR_HANDLING_MAX_ATTEMPTS:
            logger.info(
                "Command execution failed, routing to code error handling node."
            )
            state.command_error_handler_active = True
            return state
        else:
            logger.info(
                "Could not resolve the error after maximum attempts, moving to the next step."
            )
