...
from oshope.core.graphs.execution.parallel.nodes.step_resolver import step_resolver_node
from oshope.tools.command_execution import run_command
from oshope.utils.logger import get_logger
from oshope.config.config import COMMAND_ERROR_HANDLING_MAX_ATTEMPTS
...
from oshope.core.graphs.execution.parallel.nodes.code_error_handling import (
...
def analyze_code_execution(
    ...
    if not command_execution.success:
        if (
            state.planning.plan_steps[step_index].num_error_executions
...
def code_execution_node(
    state: OSHopeState, step: Step, coordinator: CommandBatchCoordinator
):
    """
        ...
        with ThreadPoolExecutor(max_workers=len(commands)) as executor:
            ...
            for future in as_completed(futures):
                ...
                try:
                    ...
                    logger.exception(f"Analyze code execution failed: {e}")
    else:
        analyze_code_execution(
            state,
            commands[0],
            coordinator_output[0][0],
            coordinator_output[0][1],
            coordinator_output[0][2],
            step_index,
        )
